...
interface IPackingProductDetailsProps {
...
function PackingProductDetails({
  watch,
  register,
  setValue,
  control,
  formState: { errors, dirtyFields },
}: IPackingProductDetailsProps) {
    ...
    <CollapseWrapper title="جزئیات محصول" className="flex flex-col gap-y-5">
      ...
      <div className="flex items-end gap-x-4">
        <Input
          ...
          label="وزن محصول"
          className="input input-bordered w-full h-[46px]"
          error={errors.weight?.value}
          iconEnd={
            dirtyFields.weight?.value ? (
              <button
                type="button"
                className="absolute end-4 inset-y-auto text-grey-600"
                onClick={() => setValue("name", "")}
