    ...
    <div className="h-innerContainer flex items-stretch gap-x-4">
      <div className="w-7/12 h-full flex flex-col border border-grey-200 rounded-custom">
        <div className="py-3 px-5 flex items-center bg-grey-50 rounded-t-custom gap-x-2">
          ...
          <span>محمد رشیدی</span>
          <button className="btn btn-ghost text-grey-800 ms-auto">
            <Show />
          </button>
        </div>
        <div className="h-full flex flex-col gap-y-4 overflow-auto p-5">
          {ticketsReply?.data.results.map((item: ticket_reply) => {
            if (item.is_admin_response)
              return (
                <div className="w-full p-3.5 bg-grey-50 rounded-custom rounded-tr-none flex flex-col gap-y-2">
            ...
            return (
              <div className="w-full p-3.5 bg-success-50 rounded-custom rounded-tl-none flex flex-col gap-y-2">
                <span>{item.message}</span>
                {item.file ? (
                  <a
                    href={item.file}
        ...
        >
          ...
          <Input
            ...
            iconEnd={
              <label
                htmlFor="file"
                className="absolute end-0 inset-y-auto text-grey-800 btn btn-ghost"
              >
                <Attach />
              </label>
            }
